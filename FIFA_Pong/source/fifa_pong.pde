...
void setup()
{
   size(1200, 700); //width = 1200 et height = 700
   background(0);
...
void rebondir()
{
   ...
   if (posX_pong1+l_pong-rBall<x        &&  x+speedX<=posX_pong1+l_pong  &&  posY_pong1<y                && y<posY_pong1+h_pong) { if ( posY_pong1 + (h_pong / 2) < y ) { speedY+=0.1; } else { speedY-=0.1; } x=posX_pong1+l_pong+rBall; speedX = abs(speedX); speedX++; }
   ...
   else if (posX_pong2-rBall<x+speedX    &&  x<=posX_pong2                &&  posY_pong2<y               && y<posY_pong2+h_pong) { x=posX_pong2-rBall;speedX = -abs(speedX); speedX--; if ( posY_pong1 + (h_pong / 2) < y ) { speedY+=0.1; } else { speedY-=0.1; }}
